...
const EDITIONS = {
  ...
  brawlstars: { name: 'Brawl Stars', words: [
    {word:'Shelly',clue:'Default brawler that shoots a wide shotgun blast'},
    ...
    {word:'Sandy',clue:'Legendary who puts enemies to sleep with sandstorms'},
  ...
  general: { name: 'General', words: [
    ...
    {word:'Chess',clue:'Strategy board game played with 16 pieces per side'},
    {word:'Submarine',clue:'Vehicle that travels underwater for long periods'},
    {word:'Guitar',clue:'Stringed instrument played by strumming or picking'},
    {word:'Dinosaur',clue:'Ancient reptiles that lived millions of years ago'},
    {word:'Rainbow',clue:'Colorful arc in the sky formed by light and rain'}
  ]}
};
...
function nextDrawTurn(room) {
  ...
  if (room.round >= room.totalRounds) {
    ...
    io.to(room.code).emit('turnChange', {
      ...
      totalRounds: room.totalRounds, currentPlayer: room.players[room.currentTurn]
    });
  }
}
...
io.on('connection', (socket) => {
  ...
  socket.on('startGame', () => {
    ...
    if (!room) return;
    const host = getPlayer(room, socket.id);
    ...
    room.players.forEach(p => {
      ...
      io.to(p.id).emit('gameStarted', {
        isImpostor: imp, word: imp ? null : room.word,
        hint: imp ? null : room.hint, wordData: imp ? null : room.wordData,
        players: room.players, currentPlayer: room.players[0],
        round: 0, totalRounds: room.totalRounds,
        gameMode: room.gameMode, edition: room.edition
      });
    });
  });
  ...
  socket.on('submitVote', ({ votedId }) => {
    ...
    if (Object.keys(room.votes).length === room.players.length) {
      ...
      const impostor = getPlayer(room, room.impostorId);
      if (caught) {
        room.phase = 'impostorGuess';
        io.to(code).emit('phaseChange', { phase: 'impostorGuess', impostorCaught: true, impostor, votedOutId: votedOut });
      } else {
        room.phase = 'results';
        io.to(code).emit('gameResults', { impostorWon: true, impostorId: room.impostorId, impostor, word: room.word, reason: 'evaded' });
      }
    }
  });
